...
ruleflow-group "${populationCriteria}"
	...
	<#elseif queryVariable.furtherDepedent == false>
		$result : ProportionResult($episodeQdmElementId : episodeId)
		$audit : ConditionAudit(sequenceId == ${conditionNo}, result == $result)
		<#else>
		$audit : ConditionAudit(sequenceId == ${conditionNo})
	</#if>
	...
	<#else>
		...
		modify ($audit) {
			...
			addQdmElementReference(new QdmElementReference($audit, $qdmElementId))
		}
		<#assign ops = "false">
		<#assign bdate = "false">
